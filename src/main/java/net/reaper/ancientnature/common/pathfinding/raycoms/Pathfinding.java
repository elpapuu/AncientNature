...
public final class Pathfinding {
    ...
    public static ThreadPoolExecutor getExecutor() {
        if (executor == null) {
            executor = new ThreadPoolExecutor(1, 5, 10, TimeUnit.SECONDS, jobQueue, new IafThreadFactory());
        }
        return executor;
    }
    ...
    public static class IafThreadFactory implements ThreadFactory {
        ...
        public Thread newThread(final @NotNull Runnable runnable) throws RuntimeException {
            ...
            if (thread.getContextClassLoader() != classLoader) {
                AncientNature.LOGGER.info("Corrected CCL of new Ice and Fire Pathfinding Thread, was: " + thread.getContextClassLoader().toString());
                thread.setContextClassLoader(classLoader);
            }
            thread.setUncaughtExceptionHandler((thread1, throwable) -> AncientNature.LOGGER.error("Ice and Fire Pathfinding Thread errored! ", throwable));
            return thread;
        }
    }
}