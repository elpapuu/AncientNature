...
import net.reaper.ancientnature.common.messages.packets.MessageMultipartInteract;
...
public abstract class EntityMultiPart extends Entity {

    private static final EntityDataAccessor<Optional<UUID>> PARENT_UUID = SynchedEntityData.defineId(EntityMultiPart.class, EntityDataSerializers.OPTIONAL_UUID);
    private static final EntityDataAccessor<Float> SCALE_WIDTH = SynchedEntityData.defineId(EntityMultiPart.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Float> SCALE_HEIGHT = SynchedEntityData.defineId(EntityMultiPart.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Float> PART_YAW = SynchedEntityData.defineId(EntityMultiPart.class, EntityDataSerializers.FLOAT);
    ...
    protected EntityMultiPart(EntityType<?> t, Level world) {
        super(t, world);
    ...
    public EntityMultiPart(EntityType<?> t, Entity parent, float radius, float angleYaw, float offsetY, float sizeX,
    ...
    public void collideWithNearbyEntities() {
        ...
        if (parent != null) {
            entities.stream().filter(entity -> entity != parent && !sharesRider(parent, entity) && !(entity instanceof EntityMultiPart) && entity.isPushable()).forEach(entity -> entity.push(parent));
    ...
    public @NotNull InteractionResult interact(@NotNull Player player, @NotNull InteractionHand hand) {
        ...
        if (level().isClientSide && parent != null) {
            NetworkHandler.sendMSGToServer(new MessageMultipartInteract(parent.getId(), 0));
        }
    ...
    public boolean hurt(@NotNull DamageSource source, float damage) {
        ...
        if (level().isClientSide && source.getEntity() instanceof Player && parent != null) {
            NetworkHandler.sendMSGToServer(new MessageMultipartInteract(parent.getId(), damage * damageMultiplier));
        }
